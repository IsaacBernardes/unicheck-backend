...
def createinvite_resolver(request, context=None):
    conn = Connection()
    ...
    try:
        ...
        query = """INSERT INTO public."unity_users" ("id_unity", "id_user", "id_occupation", "accepted")
                   SELECT %(id_unity)s, %(id_user)s, %(id_occupation)s, FALSE
                   WHERE NOT EXISTS (
                        SELECT uu."id_user" FROM public."unity_users" uu
                        WHERE uu."id_unity" = %(id_unity)s
